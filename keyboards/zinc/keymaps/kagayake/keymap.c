...
enum custom_keycodes {
  ...
  AQOUR2, // mod of Sunshine pika-pika mode : LED animation that flows from left split to right split
};

#ifdef RGBLIGHT_ENABLE
  //9色に変化するLEDのHSV各パラメータ
  //void sethsv(uint16_t hue, uint8_t sat, uint8_t val, LED_TYPE *led1) 
  uint16_t aqours_h[] = { 26, 340, 150,   0, 199, 220,  53, 265, 322};
  uint8_t aqours_s[] = {255, 165, 255, 255, 255, 94, 255, 255, 255};
  //int aqours_s[] = {255, 165, 255, 255, 255, 350, 255, 255, 255};
  uint8_t aqours_v[] = {255, 255, 255, 255, 255, 255, 255, 255, 255};
  const uint16_t NEXT_COLOR_TIME = 1211; //次の色に切り替わるまでの時間
  const uint16_t NEXT_CHANGE_TARGET_TIME = 100; //次のキーに色が伝播するまでの時間
  bool aqours_mode = false;
  bool aqours_mode2 = false;
  uint8_t aqours_num  = 0;
...
[_QWERTY] = LAYOUT_ortho_4x12(
   ...
   */
    [_ADJUST] =  LAYOUT_ortho_4x12( \
      ...
      AQOUR2, RGB_MOD, RGB_HUD, RGB_SAD, RGB_VAD, _______,                   _______, _______, _______, _______, _______, _______, \
...
bool process_record_user(uint16_t keycode, keyrecord_t *record) {
  ...
  void aqours_led(void) {
    //一定間隔で色が変化
    static uint8_t arrayLength = sizeof(aqours_s) / sizeof(uint8_t);
    if (timer_elapsed(color_timer) > NEXT_COLOR_TIME) {
      aqours_num++;
      color_timer = timer_read();
      target_col = 0;
      if (aqours_num == arrayLength) {
        aqours_num = 0;
      }
    }

    //キー毎に時間差で色が変化していく
    if (timer_elapsed(change_target_timer) > NEXT_CHANGE_TARGET_TIME) {
      if (target_col < MATRIX_COLS) {
        for (uint8_t i = 0 ; i<MATRIX_ROWS/2 ; i++) {
          uint8_t color_index = aqours_num + isLeftHand;
          if (color_index == arrayLength) {
            color_index = 0;
          }
          sethsv(aqours_h[color_index], aqours_s[color_index], aqours_v[color_index], (LED_TYPE *)&led[led_layout[target_col + i*MATRIX_COLS]]);
        };
        target_col++;
        rgblight_set();
        change_target_timer = timer_read();
      }
    }
  }

  void aqours_led2(void) {
    //一定間隔で色が変化
    if (timer_elapsed(color_timer) > NEXT_COLOR_TIME) {
      aqours_num++;
      color_timer = timer_read();
    ...
    //キー毎に時間差で色が変化していく
    if (timer_elapsed(change_target_timer) > NEXT_CHANGE_TARGET_TIME) {
      if (target_col < MATRIX_COLS*2) {
        if ((isLeftHand && (target_col < MATRIX_COLS)) || (!isLeftHand && (target_col >= MATRIX_COLS))) {
          for (int i = 0 ; i<MATRIX_ROWS/2 ; i++) {
          sethsv(aqours_h[aqours_num], aqours_s[aqours_num], aqours_v[aqours_num], (LED_TYPE *)&led[led_layout[target_col - (isLeftHand ? 0 : MATRIX_COLS) + i*MATRIX_COLS]]);
          };
        rgblight_set();
        }
        target_col++;
